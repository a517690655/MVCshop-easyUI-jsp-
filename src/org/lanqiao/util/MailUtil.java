...
public class MailUtil {
	...
	static final String AUTH_PASSWORD="aa123456";
	static final String FROM="13123373329@163.com";//发送邮件的账号（公司固定账号）
	...
	public static void sendMail(String go,String subject,String content){
		...
		try {
			message.setFrom(new InternetAddress(FROM,"超级书店管理员","utf-8"));
			message.setSubject("这是一封激活注册账号的邮件");
			message.setSentDate(new Date());//设置发送日期；
			message.setContent(content,"text/html;charset=utf-8");
			//设置收件人
			message.setRecipient(MimeMessage.RecipientType.TO, new InternetAddress(go, "社会你棠哥", "utf-8"));//发件
